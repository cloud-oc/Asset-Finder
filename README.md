...
> 提示：若未勾选任何格式筛选，搜索不会额外限制扩展名；如需限定格式，请在“文件格式筛选”中选择至少一种。

---

## 主题与大目录处理 🌓
- 顶部主题按钮支持深色 / 浅色主题切换。首次打开会跟随系统偏好，手动选择会保存在当前浏览器中。
- 导入阶段只处理浏览器提供的文件元数据，不会读取文件内容。进度以“已处理文件数 / 文件总数”为确定性指标，同时显示容量、处理速度和预计剩余时间。
- 大目录索引和搜索采用分块处理，页面会持续响应；索引或搜索过程中可以点击“取消”。取消索引会保留上一次可用索引，取消搜索会保留原搜索结果。
- 未选择任何格式时，搜索不会额外限制扩展名；如需限定格式，请在“文件格式筛选”中选择至少一种。
- 目录选择依赖 `webkitdirectory`，推荐使用 Chrome、Edge 或其他 Chromium 浏览器。
